...
import { UploadCloud, CheckCircle, AlertTriangle, XCircle, ArrowRight, RefreshCcw, Package, Loader2 } from 'lucide-react';
import Papa from 'papaparse';
import * as XLSX from 'xlsx';
import api from '../api/client';

const REQUIRED_FIELDS = [
    { key: 'nome', label: 'Nome do Produto', type: 'string', required: true },
    { key: 'codigo_sku', label: 'Código SKU', type: 'string', required: false },
    { key: 'categoria', label: 'Categoria', type: 'string', required: false, default: 'Geral' },
    { key: 'preco_venda', label: 'Preço de Venda', type: 'number', required: false, default: 0 },
    { key: 'custo_unitario', label: 'Custo Unitário', type: 'number', required: false, default: 0 },
    { key: 'estoque_atual', label: 'Estoque Atual', type: 'number', required: false, default: 0 },
    { key: 'estoque_minimo', label: 'Estoque Mínimo (Alerta)', type: 'number', required: false, default: 0 }
];

function ImportProdutosModal({ isOpen, onClose, onSuccess }) {
                    ...
                    REQUIRED_FIELDS.forEach(field => {
                        ...
                        const match = fields.find(h => {
                            ...
                            return normH === normField ||
                                (field.key === 'nome' && normH === 'produto') ||
                                (field.key === 'nome' && normH === 'nome') ||
                                (field.key === 'codigo_sku' && normH === 'sku') ||
                                (field.key === 'preco_venda' && (normH === 'preco' || normH === 'valor')) ||
                                (field.key === 'estoque_atual' && (normH === 'estoque' || normH === 'quantidade'));
                        });
    ...
    const processPreview = () => {
        ...
        filteredCsvRows.forEach((row, i) => {
            ...
            REQUIRED_FIELDS.forEach(f => {
                ...
                mappedRow[f.key] = val || f.default || (f.type === 'number' ? 0 : null);
            });
    ...
    const confirmImport = async () => {
        ...
        setToastError(null);
        try {
            const res = await api.post('/produtos/import-bulk', { rows: validRows });
            setResult(res.data);
            setStep(4);
            onSuccess();
        } catch (err) {
            setToastError(err.response?.data?.error || 'Erro interno ao importar produtos.');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                    ...
                    {step === 1 && (
                        <div
                            ...
                            onClick={() => document.getElementById('csvUploadProd').click()}
                        >
                            <input
                                id="csvUploadProd"
                                type="file"
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '24px' }}>
                                <div className="glass-card" style={{ padding: '16px', textAlign: 'center' }}>
                                    ...
                                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Produtos Válidos</div>
                                </div>
                                <div className="glass-card" style={{ padding: '16px', textAlign: 'center' }}>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                                    <thead>
                                        <tr style={{ background: 'rgba(255,255,255,0.05)' }}>
                                            ...
                                            <th style={{ padding: '8px', textAlign: 'right' }}>Estoque</th>
                                            <th style={{ padding: '8px', textAlign: 'right' }}>Preço de Venda</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {validRows.slice(0, 5).map((r, i) => (
                                            <tr key={i} style={{ borderBottom: '1px solid var(--glass-border)' }}>
                                                ...
                                                <td style={{ padding: '8px', textAlign: 'right', fontWeight: 'bold', color: r.estoque_atual > 0 ? 'var(--revenue)' : 'var(--expense)' }}>{r.estoque_atual}</td>
                    ...
                    {step === 4 && result && (
                        <div style={{ textAlign: 'center', padding: '20px 0' }}>
                            ...
                            <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Seus produtos foram cadastrados com sucesso.</p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', textAlign: 'left', background: 'rgba(255,255,255,0.02)', padding: '20px', borderRadius: '12px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ color: 'var(--text-muted)' }}>Produtos Importados:</span>
